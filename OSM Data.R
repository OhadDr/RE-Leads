...
hotels <- numeric()
countmap <- 0
for (i in 1:length(london_tab$hotel_coordinates)) {
  lat <- str_extract(london_tab$hotel_coordinates[i],"(.*)[,]") %>% 
    gsub(",","",.) %>% 
    as.numeric()
  lon <- str_extract(london_tab$hotel_coordinates[i],"[,](.*)") %>% 
    gsub(",","",.) %>% 
    as.numeric()
  ...
  countmap <- countmap+1
  print(paste0(countmap,"/",length(london_tab$hotel_coordinates)," Completed"))
}   
...
hist(volume_cont)
osm_loc$volume <- ifelse(volume_cont <= 1000, "Normal",
                         ifelse(volume_cont <= 2000,"High",
